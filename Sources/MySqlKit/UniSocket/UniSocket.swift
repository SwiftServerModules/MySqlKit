...
public class UniSocket {
  ...
  public init(
    type: UniSocketType, peer: String, port: Int32? = nil,
    timeout: UniSocketTimeout = (connect: 5, read: 5, write: 5)
  ) throws {
    ...
    if type == .local {
      ...
      #if os(macOS) || os(iOS) || os(tvOS)
        var hints = addrinfo(
          ai_flags: AI_PASSIVE, ai_family: PF_UNSPEC, ai_socktype: 0, ai_protocol: 0, ai_addrlen: 0,
          ai_canonname: nil, ai_addr: nil, ai_next: nil)
      #elseif os(Linux)
        var hints = addrinfo(
          ai_flags: AI_PASSIVE, ai_family: PF_UNSPEC, ai_socktype: 0, ai_protocol: 0, ai_addrlen: 0,
          ai_addr: nil, ai_canonname: nil, ai_next: nil)
      #endif
      switch type {
      case .tcp:
        #if os(macOS) || os(iOS) || os(tvOS)
  ...
  public func send(_ buffer: Data) throws {
    guard status == .connected || status == .stateless else {
      throw UniSocketError.error(detail: "socket is \(status)")
    }
    ...
    while bytesLeft > 0 {
      let rangeLeft = Range(
        uncheckedBounds: (
          lower: buffer.index(buffer.startIndex, offsetBy: (buffer.count - bytesLeft)),
      ...
      if status == .stateless {
        rc = bufferLeft.withUnsafeBytes { (ptr: UnsafeRawBufferPointer) -> Int in
          return system_sendto(
            fd, ptr.baseAddress, bytesLeft, 0, peer_addrinfo.pointee.ai_addr,
            peer_addrinfo.pointee.ai_addrlen)
        }
      } else {
        rc = bufferLeft.withUnsafeBytes { (ptr: UnsafeRawBufferPointer) -> Int in
          return system_send(fd, ptr.baseAddress, bytesLeft, 0)
        }
      }
      if rc == -1 {
        let errstr = String(validatingUTF8: strerror(errno)) ?? "unknown error code"
